...
export async function middleware(req: NextRequest) {
  ...
  try {
    ...
    const routePermission = Object.entries(featurePermissionsMap).find(([route]) => 
      url.startsWith(route)
    )?.[1];

    if (routePermission && !hasFeaturePermission(userRole, routePermission.feature, routePermission.permission)) {
...
const featurePermissionsMap: Record<string, { feature: string; permission: 'read' | 'write' | 'manage' }> = {
